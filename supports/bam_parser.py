...
class bp_iter:
	def __init__(self, file):
		self.handle = read(file)

	def parse_bam_entry(self, bam_record):
		mdz_seg = ""
		...
		query = bam_record.sam_qname
		target = bam_record.sam_rname
		...
		#Bam is zero indexed.
		start = bam_record.sam_pos0
		
		cig = bam_record.sam_cigar_list
		
		covered_ranges = cigar_to_pos(cig, start)
		covered_ranges.parse_cig_bam()
		covered_ranges = covered_ranges.get_ranges()
		
		read_len = bam_record.sam_l_seq
...
class bam_parser:
	...
	def get_header(self):
		self.handle = read(self.file)
		for line in self.handle.file_header.split('\n'):
